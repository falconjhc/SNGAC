...
from model.sngac import SnGac as SNGAC
eps = 1e-9
...
parser.add_argument('--style_embedder_device', dest='style_embedder_device',type=str,required=True)
parser.add_argument('--style_embedder_dir', dest='style_embedder_dir', type=str, required=True)
...
def main(_):
    ...
    if not available_gpu_num == 0:
        style_embedder_device = args.style_embedder_device
    else:
        style_embedder_device = avalialbe_cpu[0]
    ...
    print("FeatureExtractorDevice:%s" % style_embedder_device)
    print("#####################################################")
    ...
    model = SNGAC(debug_mode=args.debug_mode,
                  ...
                  style_embedder_devices=style_embedder_device,

...
def encoder_framework(images,
    ...
    with tf.variable_scope(tf.get_variable_scope()):
        ...
        return output_final_encoded, encoder_layer_list, residual_input_list, return_str
...
def generator_framework(style_reference,ebdd_logits,
                        is_training,
                        batch_size,
                        generator_device,
                        residual_at_layer,
    ...
    encoded_style_final, style_short_cut_interface, style_residual_interface, _ = \
        encoder_framework(images=style_reference,
                          is_training=is_training,
                          encoder_device=generator_device,
                          residual_at_layer=residual_at_layer,
                          residual_connection_mode='Multi',
                          ...
                          weight_decay=weight_decay,
                          weight_decay_rate=weight_decay_rate)
    style_short_cut_interface[len(style_short_cut_interface)-1]=\
        tf.concat([style_short_cut_interface[len(style_short_cut_interface)-1],
                   tf.reshape(ebdd_logits,shape=[int(ebdd_logits.shape[0]),1,1,
                                                 int(ebdd_logits.shape[1])])], axis=3)


    # fused resudual interfaces are put into the residual blocks
    if not residual_block_num == 0 or not residual_at_layer == -1:
        residual_output_list, _ = residual_block(input_list=style_residual_interface,
                                                 residual_num=residual_block_num,
                                                 residual_at_layer=residual_at_layer,
                                                 is_training=is_training,
                                                 residual_device=generator_device,
    ...
    # combination of all the encoder outputs
    style_short_cut_interface.reverse()
    encoded_layer_list = style_short_cut_interface
    encoded_layer_list.extend(residual_output_list)


    return_str = ("GeneratorEncoderDecoder %d Layers with %d ResidualBlocks connecting %d-th layer"
                  % (int(np.floor(math.log(int(style_reference[0].shape[1])) / math.log(2))),
                     residual_block_num,
                     residual_at_layer))

    # decoder part
    img_width = int(style_reference.shape[1])
    img_filters = int(style_reference.shape[3])
    generated_img, _ = \
        decoder_framework(encoded_layer_list=encoded_layer_list,
                          is_training=is_training,
                          output_width=img_width,
    ...
    return generated_img, encoded_style_final, return_str
...
def vgg_16_net(image,
    ...
    with tf.variable_scope(usage_scope):
        ...
        return output_label1, output_label0, return_str